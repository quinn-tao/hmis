...
func TestParseDateToken(t *testing.T) {
    ...
    tcs := []struct{
        ...
        }, 
        {
            ...
            InputTokens: []string{"2000-02-01", "00-02-01", "02/01/2000", "02/01/00"},
            ExpErr: false,
            ExpDate: time.Date(2000, time.January, 2, 0, 0, 0, 0, time.Local),
        },
        {
        ...
        }, 
        {
            ...
            InputTokens: []string{"0201", "24-24", "1", "011"},
            ExpErr: true,  
            ExpDate: invalidDate,
        },
        { 
    ...
    for _, tc := range tcs {
        ...
        for _, token := range tc.InputTokens { 
            ...
            if actErr != nil && !tc.ExpErr {
                t.Fatalf("Expect no error, got %v", actErr)
            }

            if !isSameDate(actDate, tc.ExpDate) {
                t.Fatalf("Expecting %v, got %v", tc.ExpDate, actDate)
            } 
        }
    }
}
